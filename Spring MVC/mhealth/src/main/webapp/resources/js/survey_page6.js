...
function getSurvey() {
	$.ajax(
			{
				url: contextPath+"/resources/js/baseline_survey_page6_data.json",
				type: "GET",
				...
				success: function(data){
					...
					if(data.status == 0){
						...
						if(survey.indexOf("baseline") > -1) {
							window.location.href = contextPath+"/survey_baseline/7";
						} else {
							window.location.href = contextPath+"/survey_sixweek/7";
						}
					} else {
